...
def cadastrar_aluno_flow():
    ...
    Campos: nome, email, genero, data_nascimento (YYYY-MM-DD), senha, confirma senha
    """
    from integrated_data_store import DataStore, encrypt_text
    import getpass, re
    data = DataStore.get_instance()
    print("="*40)
    print("Sys Academy - CADASTRO DE ALUNO (AUTO)")
    print("="*40)
    ...
    if senha != senha2:
        ...
        return None
    # build aluno and usuario records
    aluno = {"nome": nome, "email": email, "genero": genero, "data_nascimento": dob}
    try:
    ...
    usuario = {"nome": nome, "email": email, "role": "aluno", "senha": senha, "genero": genero, "data_nascimento": dob, "aluno_id": aluno.get("id") if isinstance(aluno, dict) else None}
    try:
        data.cadastrar_usuario(usuario)
    except Exception as e:
        print("Erro ao criar conta de acesso:", e)
    # mostrar confirmação
    criado_aluno = next((a for a in data.alunos if a.get("email")==email), None)
    criado_user = next((u for u in data.usuarios if u.get("email")==email), None)
    if criado_aluno:
